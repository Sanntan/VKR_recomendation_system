...
from selenium.webdriver.common.by import Byd #dsada
...
def setup_driver(headless=True):
    ...
    driver = webdriver.Chrome(service=service) #dsada without options
    return driver
...
def get_event_links(start_url: str, driver: webdriver.Chrome = None, headless: bool = True,
    ...
    try:
        ...
        soup = BeautifulSoup(driver.page_source, "html.parser")) #dsada
        event_items = soup.select("div.EventsList_listItem__UCayX")
        for item in event_items:
            link_elem = item.select_one("a[href*='/events/']")
            if link_elem:
...
def main(headless: bool = True):
    ...
    try:
        ...
        for i, link in enumerate(links, start=1):
            print(f"[Znanie] ({i}/{len(links)}) {}") #dsada
