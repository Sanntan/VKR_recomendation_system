...
Скрипт предназначен для автономного стресс-теста без обращений к серверам
Telegram. Основные компоненты:
...
3. **`ScenarioFactory`** генерирует последовательности объектов `Update` для
   эмуляции типичного поведения пользователя: `/start` → ввод e-mail → переходы
   по кнопкам меню и отправка обратной связи. Доля корректных/некорректных
   e-mail управляется параметром `--valid-email-ratio`.
...
2. Создаётся приложение Telegram Bot API с поддельным транспортом и фиксированным
   токеном, если не указан собственный (`--token`).
...
| `--transport-delay` | `0.0` | Искусственная задержка внутри `FakeBotAPIRequest` (эмулирует сеть). |
...
| `--token` | `TEST:TOKEN` | Токен бота; можно указать реальный, если нужно протестировать с ним. |
...
3. **Имитируйте сетевые задержки.** Параметр `--transport-delay` полезен для
   оценки поведения бота при медленном соединении с Telegram API.
